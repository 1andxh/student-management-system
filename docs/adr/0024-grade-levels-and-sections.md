...
- `ClassService.update` can still change `term_id` on an attached class, re-opening the year mismatch after the fact. Not fixed here because the clean fix re-introduces the `classes → sections` import direction this stage deliberately avoided; the cheaper alternative is to reject `term_id` changes outright while `section_id` is set. **Open decision — raised with the user, not silently accepted.**
